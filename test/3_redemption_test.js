...
const seeding = require('./helpers/seed_rinkeby');
...
contract('Redenption Tests', (ACCOUNTS) => {
  ...
  const TOKEN_HOLDER = [ACCOUNTS[15]];
  ...
  describe("OPERATOR", () => {
    ...
    describe("Issue token - (#issue)", () => {
      it("", async () => {
        const txHash = await SecurityToken.issue(
          TOKEN_HOLDER[0],
          DEFAULT_TOKEN_VALUE,
          ZERO_32BYTES,
          {from: ISSUER_1}
    ...
    describe("Redeem token - (#redeemFrom)", () => {
    ...
    describe("Set partition - (#setPartition)", () => {
      it("", async () => {
        await Partitions.setPartition(
          seed.partitionValues[0].name,
          seed.partitionValues[0].data,
          {from: ISSUER_1}
        );
      });
    });

    describe("Issue token 2 - (#issue)", () => {
      it("", async () => {
        const txHash = await SecurityToken.issue(
          TOKEN_HOLDER[0],
          DEFAULT_TOKEN_VALUE,
