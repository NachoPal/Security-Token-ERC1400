...
library Utils {
  ...
  /** @dev Transform bytes32 to string to get reason values from Rules contract
    * @param reason .
    */
  function toString(bytes32 reason) internal pure returns(string memory){
      uint256 counter = 0;
      ...
      bytes memory bytesArray = new bytes(counter);

      for (uint256 j; j < counter; j++) {
          bytesArray[j] = reason[j];
      }
